/*
Template Name: Material Pro admin
Author: Wrappixel
Email: niravjoshi87@gmail.com
File: js
*/
$(function() {
    "use strict";
      $(".tst1").click(function(){
           $.toast({
            heading: 'Welcome to Material Pro admin',
            text: 'Use the predefined ones, or specify a custom position object.',
            position: 'top-right',
            loaderBg:'#ff6849',
            icon: 'info',
            hideAfter: 3000, 
            stack: 6
          });

     });

      $(".tst2").click(function(){
           $.toast({
            heading: 'Welcome to Material Pro admin',
            text: 'Use the predefined ones, or specify a custom position object.',
            position: 'top-right',
            loaderBg:'#ff6849',
            icon: 'warning',
            hideAfter: 3500, 
            stack: 6
          });

     });

      $(".tst3").click(function(){
           $.toast({
            heading: 'Welcome to Material Pro admin',
            text: 'Use the predefined ones, or specify a custom position object.',
            position: 'top-right',
            loaderBg:'#ff6849',
            icon: 'success',
            hideAfter: 3500, 
            stack: 6
          });

     });

      $(".tst4").click(function(){
           $.toast({
            heading: 'Welcome to Material Pro admin',
            text: 'Use the predefined ones, or specify a custom position object.',
            position: 'top-right',
            loaderBg:'#ff6849',
            icon: 'error',
            hideAfter: 3500
            
          });

     });
     
     //stage Display Start
     $(".UpdateAppt").click(function(){
          $.toast({
           heading: '',
           text: 'Ravi Agrawal updated an appointment time. ',
           position: 'top-right',
           loaderBg:'#ff6849',
           icon: 'warning',
           hideAfter: 3500, 
           stack: 6
         });

         $.toast({
          heading: '',
          text: 'Ravi Agrawal updated an appointment time. ',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
        });

        $.toast({
         heading: '',
         text: 'Ravi Agrawal updated an appointment time. ',
         position: 'top-right',
         loaderBg:'#ff6849',
         icon: 'error',
         hideAfter: 3500, 
         stack: 6
       });

    });

    $(".uaLocationInfo").click(function(){
     $.toast({
      heading: '',
      text: 'Ravi Agrawal updated location info. ',
      position: 'top-right',
      loaderBg:'#ff6849',
      icon: 'warning',
      hideAfter: 3500, 
      stack: 6
    });

    $.toast({
     heading: '',
     text: 'Ravi Agrawal updated location info. ',
     position: 'top-right',
     loaderBg:'#ff6849',
     icon: 'success',
     hideAfter: 3500, 
     stack: 6
   });

   $.toast({
    heading: '',
    text: 'Ravi Agrawal updated location info. ',
    position: 'top-right',
    loaderBg:'#ff6849',
    icon: 'error',
    hideAfter: 3500, 
    stack: 6
  });

});

$(".reportReview").click(function(){
     $.toast({
     heading: '',
     text: 'Ravi Agrawal reported a review',
     position: 'top-right',
     loaderBg:'#ff6849',
     icon: 'warning',
     hideAfter: 3500, 
     stack: 6
     });
     
     $.toast({
     heading: '',
     text: 'Ravi Agrawal reported a review',
     position: 'top-right',
     loaderBg:'#ff6849',
     icon: 'success',
     hideAfter: 3500, 
     stack: 6
});

     $.toast({
          heading: '',
          text: 'Ravi Agrawal reported a review',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'error',
          hideAfter: 3500, 
          stack: 6
     });
});

$(".saveServiceAvailability").click(function(){
     $.toast({
     heading: '',
     text: 'Ravi Agrawal saved an availability for a service type',
     position: 'top-right',
     loaderBg:'#ff6849',
     icon: 'warning',
     hideAfter: 3500, 
     stack: 6
     });
     
     $.toast({
     heading: '',
     text: 'Ravi Agrawal saved an availability for a service type',
     position: 'top-right',
     loaderBg:'#ff6849',
     icon: 'success',
     hideAfter: 3500, 
     stack: 6
});

     $.toast({
          heading: '',
          text: 'Ravi Agrawal saved an availability for a service type',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'error',
          hideAfter: 3500, 
          stack: 6
     });
});

$(".saveServiceAppointment").click(function(){
     $.toast({
     heading: '',
     text: 'Ravi Agrawal saved a service type',
     position: 'top-right',
     loaderBg:'#ff6849',
     icon: 'warning',
     hideAfter: 3500, 
     stack: 6
     });
     
     $.toast({
     heading: '',
     text: 'Ravi Agrawal saved a service type',
     position: 'top-right',
     loaderBg:'#ff6849',
     icon: 'success',
     hideAfter: 3500, 
     stack: 6
});

     $.toast({
          heading: '',
          text: 'Ravi Agrawal saved a service type',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'error',
          hideAfter: 3500, 
          stack: 6
     });
});

$(".blockServiceAvailability").click(function(){
     $.toast({
     heading: '',
     text: 'Ravi Agrawal blocked an availability for a service type',
     position: 'top-right',
     loaderBg:'#ff6849',
     icon: 'warning',
     hideAfter: 3500, 
     stack: 6
     });
     
     $.toast({
     heading: '',
     text: 'Ravi Agrawal blocked an availability for a service type',
     position: 'top-right',
     loaderBg:'#ff6849',
     icon: 'success',
     hideAfter: 3500, 
     stack: 6
});

     $.toast({
          heading: '',
          text: 'Ravi Agrawal blocked an availability for a service type',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'error',
          hideAfter: 3500, 
          stack: 6
     });
});

$(".replyContact").click(function(){
     $.toast({
     heading: '',
     text: 'Ravi Agrawal sent a reply to a contact',
     position: 'top-right',
     loaderBg:'#ff6849',
     icon: 'warning',
     hideAfter: 3500, 
     stack: 6
     });
     
     $.toast({
     heading: '',
     text: 'Ravi Agrawal sent a reply to a contact',
     position: 'top-right',
     loaderBg:'#ff6849',
     icon: 'success',
     hideAfter: 3500, 
     stack: 6
});

     $.toast({
          heading: '',
          text: 'Ravi Agrawal sent a reply to a contact',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'error',
          hideAfter: 3500, 
          stack: 6
     });
});

     $(".mgrnotification").click(function(){
          $.toast({
               heading: '',
               text: 'Ravi Agrawal jumped visit Shakir Ansari 11/10/2020 to Stage MGR. ',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'success',
               hideAfter: 3500, 
               stack: 6
          });

     });

     $(".mgrnotification").click(function(){
        $.toast({
         heading: '',
         text: 'Ravi Agrawal jumped visit Shakir Ansari 11/10/2020 to Stage MGR. ',
         position: 'top-right',
         loaderBg:'#ff6849',
         icon: 'error',
         hideAfter: 3500, 
         stack: 6
       });

  });

     $(".locationinfoUpdate").click(function(){
          $.toast({
           heading: '',
           text: 'Ravi Agrawal updated Location Information.',
           position: 'top-right',
           loaderBg:'#ff6849',
           icon: 'warning',
           hideAfter: 3500, 
           stack: 6
         });
         
          $.toast({
          heading: '',
          text: 'Ravi Agrawal updated Location Information.',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal updated Location Information.',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
  });

     $(".stepAcknowledge").click(function(){
     $.toast({
      heading: '',
      text: 'Ravi Agrawal Acknowledged a Step',
      position: 'top-right',
      loaderBg:'#ff6849',
      icon: 'warning',
      hideAfter: 3500, 
      stack: 6
    });
    
     $.toast({
     heading: '',
     text: 'Ravi Agrawal Acknowledged a Step',
     position: 'top-right',
     loaderBg:'#ff6849',
     icon: 'success',
     hideAfter: 3500, 
     stack: 6
});

     $.toast({
          heading: '',
          text: 'Ravi Agrawal Acknowledged a Step',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'error',
          hideAfter: 3500, 
          stack: 6
     });
});

     $(".newptMarked").click(function(){
     $.toast({
      heading: '',
      text: 'Ravi Agrawal Marked a New Patient',
      position: 'top-right',
      loaderBg:'#ff6849',
      icon: 'warning',
      hideAfter: 3500, 
      stack: 6
    });
    
     $.toast({
     heading: '',
     text: 'Ravi Agrawal Marked a New Patient',
     position: 'top-right',
     loaderBg:'#ff6849',
     icon: 'success',
     hideAfter: 3500, 
     stack: 6
});

     $.toast({
          heading: '',
          text: 'Ravi Agrawal Marked a New Patient',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'error',
          hideAfter: 3500, 
          stack: 6
     });
});
    
     $(".queAddStage").click(function(){
     $.toast({
      heading: '',
      text: 'Ravi Agrawal updated a questionnaire for a Visit.',
      position: 'top-right',
      loaderBg:'#ff6849',
      icon: 'warning',
      hideAfter: 3500, 
      stack: 6
    });
    
     $.toast({
     heading: '',
     text: 'Ravi Agrawal updated a questionnaire for a Visit.',
     position: 'top-right',
     loaderBg:'#ff6849',
     icon: 'success',
     hideAfter: 3500, 
     stack: 6
});

     $.toast({
          heading: '',
          text: 'Ravi Agrawal updated a questionnaire for a Visit.',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'error',
          hideAfter: 3500, 
          stack: 6
     });
});

     $(".commentleftStage").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Left a Comment for a Visit',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Left a Comment for a Visit',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Left a Comment for a Visit',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".updateVisitStage").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a Visit',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a Visit',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated a Visit',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".textoptStage").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Sent a Text to a Patient',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Sent a Text to a Patient',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Sent a Text to a Patient',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".emailoptStage").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Sent a Text to a Patient',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Sent a Text to a Patient',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Sent a Text to a Patient',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".assignPhyStage").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Assigned a Physician to a Patient Visit',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Assigned a Physician to a Patient Visit',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Assigned a Physician to a Patient Visit',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".unassignPhyStage").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Unassigned a Physician to a Patient Visit',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Unassigned a Physician to a Patient Visit',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Unassigned a Physician to a Patient Visit',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".resourceupdateStage").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Resources for a Patient Visit',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Resources for a Patient Visit',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated Resources for a Patient Visit',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".demoupdateStage").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Demo for a Patient',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Demo for a Patient',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated Demo for a Patient',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".insuranceupdateStage").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Insurance for a Patient',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Insurance for a Patient',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated Insurance for a Patient',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".paymentreminderStage").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal sent a Payment Reminder for a Patient',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal sent a Payment Reminder for a Patient',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal sent a Payment Reminder for a Patient',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".recordnxtStage").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal sent Record to Next Stage',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal sent Record to Next Stage',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal sent Record to Next Stage',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });
     
     $(".jumptoNxtstage").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Next Stage a visit for Shakir Ansari 11/10/2020 to Stage MGR.',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Next Stage a visit for Shakir Ansari 11/10/2020 to Stage MGR.',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Next Stage a visit for Shakir Ansari 11/10/2020 to Stage MGR.',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });  
     
     $(".questionnaireUpdate").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal updated Questionnaire for patient visit',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal updated Questionnaire for patient visit',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal updated Questionnaire for patient visit',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });
     //stage Display End

     //Visit Type Start
     $(".visitReason").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a Visit Reason',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a Visit Reason',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated a Visit Reason',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".visittypeOrder").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal updated order for a Visit Type',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal updated order for a Visit Type',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal updated order for a Visit Type',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".visittypeUpdate").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a Visit type',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a Visit type',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated a Visit type',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".visitReasonOrder").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal updated order for a Visit Reason',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal updated order for a Visit Reason',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal updated order for a Visit Reason',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".visitTypeAdd").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a Visit Type',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a Visit Type',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Added a Visit Type',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".visitReasonAdd").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a Visit Reason',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a Visit Reason',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Added a Visit Reason',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });
     //Visit Type End

     //Demo Start
     $(".demofieldAdd").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a Demo Field',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a Demo Field',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Added a Demo Field',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".demoActive").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal set a Demo Field as Active',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal set a Demo Field as Active',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal set a Demo Field as Active',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });
     //Demo End

     //Questionnaire Start
     $(".questionAdd").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a Questionnaire Field',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a Questionnaire Field',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Added a Questionnaire Field',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".questionUpdate").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a Questionnaire Field',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a Questionnaire Field',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated a Questionnaire Field',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".questionActive").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal set a Questionnaire Field as Active',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal set a Questionnaire Field as Active',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal set a Questionnaire Field as Active',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".questionArchive").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal set a Questionnaire Field as Archived',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal set a Questionnaire Field as Archived',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal set a Questionnaire Field as Archived',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });
     //Questionnaire End

     //Stage and Step Start
     $(".stagenstepAdd").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a Stage',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a Stage',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Added a Stage',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".stagenStageAdd").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal updated a Stage',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal updated a Stage',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal updated a Stage',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".stageUpdate").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a Stage',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a Stage',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated a Stage',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".stepAdd").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a Step',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a Step',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Added a Step',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".stepUpdate").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a Step',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a Step',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated a Step',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".StageOrder").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal updated order for a Stage',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal updated order for a Stage',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal updated order for a Stage',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".StepOrder").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal updated order for a Step',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal updated order for a Step',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal updated order for a Step',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".addStage").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Added a Stage',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Added a Stage',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Added a Stage',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".addResource").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal added a Resource to a Stage',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal added a Resource to a Stage',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal added a Resource to a Stage',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".addResourceStep").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal added a Resource to a Step',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal added a Resource to a Step',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal added a Resource to a Step',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });
     //Stage and Step End

     //Resource Start
     $(".resourceAdd").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a Resource',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a Resource',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Added a Resource',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".resourceUpdate").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a Resource',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a Resource',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated a Resource',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".resourceArchive").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Archived a Resource',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Archived a Resource',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Archived a Resource',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });
     //Resource End

     //SMS Template Start
     $(".smstempAdd").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a SMS Template',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added a SMS Template',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Added a SMS Template',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".smstempUpdate").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a SMS Template',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated a SMS Template',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated a SMS Template',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".smstempDelete").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Deleted a SMS Template',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Deleted a SMS Template',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Deleted a SMS Template',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });
     //SMS Template End

     //Email Template Start
     $(".emailtempAdd").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added an Email Template',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Added an Email Template',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Added an Email Template',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".emailtempUpdate").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated an Email Template',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated an Email Template',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated an Email Template',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".emailtempDelete").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Deleted an Email Template',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Deleted an Email Template',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Deleted an Email Template',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });
     //Email Template End
     
     //Location Mangement Start
     $(".ptAllocation").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Patient Allocation for a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Patient Allocation for a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated Patient Allocation for a Location',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".userAllocation").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated User Allocation for a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated User Allocation for a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated User Allocation for a Location',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".smsAllocation").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated SMS Allocation for a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated SMS Allocation for a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated SMS Allocation for a Location',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".emailAllocation").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Email Allocation for a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Email Allocation for a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated Email Allocation for a Location',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".paymentAllocation").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Authorize.net API for a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Authorize.net API for a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated Authorize.net API for a Location',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".updateuserLocation").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Users for a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Users for a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated Users for a Location',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".updateptmgrLocation").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Patient Management for a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated Patient Management for a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated Patient Management for a Location',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".deactiveLocation").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Deactivated a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Deactivated a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Deactivated a Location',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Activated a Location',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'warning',
               hideAfter: 3500, 
               stack: 6
          });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Activated a Location',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'success',
               hideAfter: 3500, 
               stack: 6
          });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Activated a Location',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".deactivePhysician").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Deactivated a Physician',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Deactivated a Physician',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Deactivated a Physician',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Activated a Physician',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'warning',
               hideAfter: 3500, 
               stack: 6
          });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Activated a Physician',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'success',
               hideAfter: 3500, 
               stack: 6
          });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Activated a Physician',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });
     
     $(".resendmgrLocation").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi resent access request to owner',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi resent access request to owner',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi resent access request to owner',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });
     //Location Mangement End

     //Add New Practices Start
     $(".newPractice").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Adding a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Adding a Location',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Adding a Location',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     //Add New Practices End

     //Billing Start
     $(".billingUpdate").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated the Billing Plan',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated the Billing Plan',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated the Billing Plan',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     //Billing End

     //Authorized User Start
     $(".locationaccessUser").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated the Location access for a User',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Updated the Location access for a User',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Updated the Location access for a User',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });

     $(".auPractice").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal revoked access for a location for user',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
          });
          
          $.toast({
          heading: '',
          text: 'Ravi Agrawal revoked access for a location for user',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal revoked access for a location for user',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });
     //Authorized User End

     //Active requests Start
     $(".activeRequestCredt").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Authorized a Credit Card Payment',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Authorized a Credit Card Payment',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Authorized a Credit Card Payment',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });
     
     $(".activeRequestCash").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Authorized a Cash Payment',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Authorized a Cash Payment',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Authorized a Cash Payment',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     }); 

     $(".activeRequestCheque").click(function(){
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Authorized a Cheque Payment',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'warning',
          hideAfter: 3500, 
          stack: 6
     });
     
          $.toast({
          heading: '',
          text: 'Ravi Agrawal Authorized a Cheque Payment',
          position: 'top-right',
          loaderBg:'#ff6849',
          icon: 'success',
          hideAfter: 3500, 
          stack: 6
     });

          $.toast({
               heading: '',
               text: 'Ravi Agrawal Authorized a Cheque Payment',
               position: 'top-right',
               loaderBg:'#ff6849',
               icon: 'error',
               hideAfter: 3500, 
               stack: 6
          });
     });
     //Active requests End
  
});
          
